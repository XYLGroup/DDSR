...
def gen_oof_kernel_fixed(k_size, scale_factor, lambda_1):

    raw_kernel = np.zeros(k_size)
    kc = k_size[0]//2 - 1
    for i in range(k_size[0]):
        for j in range(k_size[1]):
            if (i-kc)**2 + (j-kc)**2 < lambda_1**2:
                raw_kernel[i,j] = 1
...
def gen_oof_kernel_random(k_size, scale_factor, min_var, max_var, noise_level):
    lambda_1 = min_var + np.random.rand() * (max_var - min_var)

    kernel = gen_oof_kernel_fixed(k_size, scale_factor, lambda_1)
...
def generate_dataset(images_path, out_path_im, out_path_ker, k_size, scale_factor, min_var, max_var, noise_ker,
    ...
    for i, path in enumerate(files_source):
        ...
        # # for random anisotropic Gaussian kernel
        if kernel_type == 'Gaussian':
            kernel = gen_kernel_random(k_size, scale_factor, min_var, max_var, noise_ker)
        # for random out-of-focus kernel
        elif kernel_type == 'out_of_focus':
            kernel = gen_oof_kernel_random(k_size, scale_factor, min_var, max_var, noise_ker)
        ...
        # plt.imsave('%s/%s.png' % (out_path_ker, os.path.splitext(os.path.basename(path))[0]), kernel, vmin=0, vmax=1., dpi=1)

        plot_kernel(kernel, '%s/%s.png' % (out_path_ker, os.path.splitext(os.path.basename(path))[0]))
        kernel = read_image(os.path.join(out_path_ker, '%s.png' % os.path.splitext(os.path.basename(path))[0]))
        kernel_crop = kernel[49:351, 140:447, :]    #BSRkernal
        plt.imsave('%s/%s.png' % (out_path_ker, os.path.splitext(os.path.basename(path))[0]), kernel_crop, vmin=0, vmax=1., dpi=1)
...
def main():
    ...
    parser.add_argument('--model', type=str, default='DDSR',
                        help='DDSR, generate data blurred by anisotropic Gaussian kernel.')
    parser.add_argument('--sf', type=int, default=4, help='scale factor: 2, 3, 4, 8')
    parser.add_argument('--kernel_type', type=str, default='Gaussian', help='kernel_type: Gaussian, out_of_focus')
    ...
    if opt.model == 'DDSR':
        ...
        generate_dataset(images_path, out_path_im, out_path_ker, k_size, np.array([opt.sf, opt.sf]), min_var, max_var,
                         opt.noise_ker, opt.noise_im, opt.kernel_type)
    else:
