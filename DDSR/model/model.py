...
from .kernel_generate import gen_kernel_random_oof, gen_kernel_random, gen_kernel_random_motion, make_gradient_filter, ekp_kernel_generator
...
class DIPFKP:
    ...
    def MCMC_sampling(self):
        ...
        if self.conf.kernel_type == 'Gaussian':
            kernel_random = gen_kernel_random(self.k_size, self.sf, self.min_var, self.max_var, 0, self.conf.kernel_x,
                                              self.conf.kernel_y)

        elif self.conf.model == 'oof':
            kernel_random = gen_kernel_random_oof(self.k_size, self.sf, self.min_var, self.max_var, 0, self.conf.kernel_x,
                                              self.conf.kernel_y)
    ...
    def MLMC_warm_up(self):
        if (self.conf.model == 'DDSR'): # and iteration == 0
    ...
    def print_and_output_setting(self):
        ...
        # fold = '../data/log_MLMC/Tensorboard'
        ...
        for self.iteration in tqdm.tqdm(range(self.conf.max_iters), ncols=60):
            ...
            if self.conf.model == 'DDSR':
                ...
                for k_p in range(self.conf.D_loop):
                    ...
                    if self.conf.degra_type == "inpainting" or self.conf.degra_type == "dark":
                        out_x = out_x * self.conf.degra_matrix
                ...
                for i_p in range(self.conf.I_loop_x):
                    ...
                    if self.iteration <= 80:  #80 for FKPDIP
                    ...
                    if self.conf.degra_type == "inpainting" or self.conf.degra_type == "dark":
                        out_k = out_k * self.conf.degra_matrix
